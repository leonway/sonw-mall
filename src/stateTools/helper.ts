import { Path, ObjAction } from './types'
/**
 ...
 */
const deepAssignObj = (obj:object, subObj:ObjAction, path:Path) => {
  const { ...newObj } = obj;
 ...
 * (不会修改原对象) 为对象的关键路径赋值
 */
const deepAssignValue = (obj:object, value:any, path:Path) => {
 ...
 */
const getPathValue = (obj:object, path:Path) => {
  if (!path) {
    return undefined;
  } else {
    return makeArray(path).reduce(
      (value, key) => (typeof value !== 'object' ? undefined : value[key]),
      obj,
    );
  }
};
